...
describe('Elements verification in responsive mode for Seeker page', ()=>{
    ...
    context('Iphone 12Pro resolution', () => {
        ...
        it('Verify Elements in Seeker Dashboard after Login', ()=>{
            ...
            cy.get('.col-md-9 > :nth-child(3) > h3').should('be.visible').and('have.text','Your Classes:')
            cy.get(':nth-child(4) > h3').should('be.visible').and('have.text','Your Workshops:')
            cy.get(':nth-child(5) > h3').should('be.visible').and('have.text','Your 1-1 Sessions:')
            cy.get(':nth-child(6) > h3').should('be.visible').and('have.text','Your Seminar Events:')
