...
Cypress.on('uncaught:exception', (err, runnable) => {
  ...
  describe('Verify the Referral Link and Popup', ()=>{
    ...
    it('Verify the Referral Popup',()=>{
        ...
        cy.get('.col-lg-7 > .nav > :nth-child(2) > .nav-link').click({force:true}) //Verify Facebook button in the modal
        cy.get('#r-share-facebook').should('contain', 'Share on Facebook')
        
        cy.get('.col-lg-7 > .nav > :nth-child(3) > .nav-link').click({force:true}) //Verify Twitter button in the modal
